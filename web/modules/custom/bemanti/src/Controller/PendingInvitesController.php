...
class PendingInvitesController extends ControllerBase {
  ...
  public function pendingInvites() {
    ...
    $invites_query = \Drupal::entityQuery('user_group_invite_entity')->condition('email', $user_data->getEmail());
    
    $invites_list = $invites_query->execute();
    
    foreach ($invites_list as $key => $id) {
      $build[] =  \Drupal::formBuilder()->getForm('Drupal\bemanti\Form\InviteAcceptForm', $id);
    }
